...
class Solution {
    ...
    public void dfs(TreeNode root,int x,int y,TreeMap<Integer,TreeMap<Integer,PriorityQueue<Integer>>> map){
        ...
        if(!map.containsKey(x)){ // check if vertical is exists or not
            map.put(x,new TreeMap<>());
        }
        if(!map.get(x).containsKey(y)){ // for level 
            map.get(x).put(y,new PriorityQueue<>());
        }
        map.get(x).get(y).offer(root.val); // we are grtting the vertical and level then put the value int the map; 
